...
parser.add_argument("--learning-rate", type=float, default=0.002, help="learning rate (step size for optimization algo)")
    ...
    while environment.getStepNumber() - stepStart < minEpochSteps and not stop:
        ...
        while not environment.isGameOver() and not stop:
            ...
            if evalWithEpsilon is None:
                epsilon = 0.1#max(.1, 1.0 - 0.9 * environment.getStepNumber() / 1e6)
            else:
